    ...
    <div className="space-y-4">
      <section className="rounded-lg border bg-white dark:bg-slate-800 p-4 space-y-3">
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <label className="text-sm">Code (optional)
            <input className="block border rounded mt-1 px-2 py-1 w-full" value={draft.code} onChange={(e) => setDraft({ ...draft, code: e.target.value })} />
          </label>
          ...
          <label className="text-sm">Category
            <select className="block border rounded mt-1 px-2 py-1 w-full" value={draft.categoryId} onChange={(e) => setDraft({ ...draft, categoryId: e.target.value })}>
          ...
          <label className="text-sm">Available
        ...
        <div className="flex gap-2">
          <button className="border rounded px-3 py-1" onClick={async () => { const saved = await saveItem(draft); setDraft(defaultDraft); toast.success(`Menu item saved (${saved.code}).`); }}>{draft.id ? 'Update Item' : 'Add Item'}</button>
          {draft.id && <button className="border rounded px-3 py-1" onClick={() => setDraft(defaultDraft)}>Cancel edit</button>}
        </div>
      </section>

      <section className="rounded-lg border bg-white dark:bg-slate-800 p-4 space-y-3">
        <h3 className="font-medium">Menu Categories</h3>
        <div className="grid md:grid-cols-3 gap-2">
          <input className="border rounded px-2 py-1" placeholder="Category name" value={categoryDraft.name} onChange={(e) => setCategoryDraft({ ...categoryDraft, name: e.target.value })} />
          <input className="border rounded px-2 py-1" type="number" placeholder="Sort order" value={categoryDraft.sortOrder} onChange={(e) => setCategoryDraft({ ...categoryDraft, sortOrder: Number(e.target.value) })} />
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={categoryDraft.isActive} onChange={(e) => setCategoryDraft({ ...categoryDraft, isActive: e.target.checked })} />
            Active
          </label>
        </div>
        <div className="flex gap-2">
          <button className="border rounded px-3 py-1" onClick={async () => { const saved = await saveCategory(categoryDraft); setCategoryDraft(defaultCategoryDraft); toast.success(`Category saved (${saved.name}).`); }}>Save Category</button>
          {categoryDraft.id && <button className="border rounded px-3 py-1" onClick={() => setCategoryDraft(defaultCategoryDraft)}>Cancel edit</button>}
        </div>
        <div className="space-y-2">
          {categories.map((category) => (
            <div key={category.id} className="border rounded p-2 text-sm flex items-center justify-between">
              <p>{category.sortOrder} · {category.name} {category.isActive ? '' : '(inactive)'}</p>
              <div className="flex gap-2">
                <button className="border rounded px-2 py-1" onClick={() => setCategoryDraft(category)}>Edit</button>
                <button className="border rounded px-2 py-1" onClick={async () => { await deleteCategory(category.id); toast.info('Category removed.'); }}>Delete</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {filtered.map((item) => (
            <div key={item.id} className="border rounded p-3 flex flex-wrap items-center justify-between gap-3 text-sm">
              <div className="flex items-center gap-3 min-w-[260px]"><Image src={item.imageUrl ?? undefined} alt={item.name} className="h-14 w-14 rounded object-cover" /><div><p className="font-medium">{item.code} · {item.name} · {formatCurrency(item.price)}</p><p className="text-[#6B7280]">Category: {categoryById.get(item.categoryId) ?? (item.categoryId || 'uncategorized')} · Updated: {new Date(item.updatedAt).toLocaleString()}</p><div className="flex gap-2 mt-1 flex-wrap"><StatusChip label={item.isAvailable ? 'Available' : 'Unavailable'} tone={item.isAvailable ? 'success' : 'warning'} /><StatusChip label={item.discount > 0 ? `${item.discount}% OFF` : 'No discount'} tone={item.discount > 0 ? 'warning' : 'neutral'} /></div></div></div>
