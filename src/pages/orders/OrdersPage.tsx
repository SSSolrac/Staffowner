...
const paymentLabel = (paid: boolean) => (paid ? 'Payment verified' : 'Awaiting verification');
                ...
                <tr className="border-t" key={order.id}>
                  ...
                  <td>
                    <div className="space-y-1">
                      <StatusChip label={`${paymentLabel(paid)} · ${order.paymentMethod}`} tone={paid ? 'success' : 'warning'} />
                      {paid && <p className="text-xs text-emerald-700">Verified by cashier</p>}
                    </div>
                  </td>
          ...
          <div className="w-full max-w-4xl rounded-lg border bg-white dark:bg-slate-800 p-4 space-y-3 max-h-[90vh] overflow-auto">
            ...
            <div className="grid md:grid-cols-2 gap-3 text-sm">
              <div className="border rounded p-3 space-y-1">
                ...
                <StatusChip label={paymentLabel(selectedOrder.paymentStatus === 'paid')} tone={selectedOrder.paymentStatus === 'paid' ? 'success' : 'warning'} />
              </div>
            ...
            <div className="border rounded p-3">
              <p className="font-medium text-sm mb-2">Receipt / payment proof</p>
              {selectedOrder.paymentMethod === 'e-wallet' && selectedOrder.paymentProofUrl && (
                <img src={selectedOrder.paymentProofUrl} alt="Payment proof" className="h-36 rounded border object-cover" />
              )}
              {selectedOrder.paymentMethod === 'e-wallet' && !selectedOrder.paymentProofUrl && (
                <p className="text-sm text-slate-500">Customer has not uploaded a payment screenshot yet.</p>
              )}
              {selectedOrder.paymentMethod !== 'e-wallet' && (
                <p className="text-sm text-slate-500">Digital proof is not required for {selectedOrder.paymentMethod} payments.</p>
              )}
            </div>
